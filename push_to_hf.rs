// Push meta-introspector unified indexes to HuggingFace
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🚀 META-INTROSPECTOR → HUGGINGFACE DATASET PUSHER");
    println!("=================================================\n");
    
    let repo = "introspector/meta-introspector";
    
    // Ensure repo exists
    println!("📦 Creating HuggingFace dataset repo...");
    Command::new("huggingface-cli")
        .args(["repo", "create", repo, "--type", "dataset", "--exist-ok"])
        .output()?;
    
    // Files to push (Parquet format)
    let files = vec![
        // Unified indexes
        ("data/indexes/files.parquet", "3M files with git provenance"),
        ("data/indexes/repos.parquet", "Git repository metadata"),
        ("data/indexes/datasets.parquet", "HF + local datasets"),
        ("data/indexes/projects.parquet", "Project metadata"),
        
        // Analysis data
        ("data/markov-analysis/markov_symbol_scores.parquet", "Symbol similarity scores"),
        ("data/markov-analysis/markov_similarity_matrix_meta.json", "Matrix metadata"),
        
        // Eigenvector analysis
        ("data/eigenvectors/word_eigenvectors.json", "Word eigenvector analysis"),
        
        // Telemetry
        ("data/telemetry/rustc_trace_schema.parquet", "Rustc compilation traces"),
        ("data/telemetry/syscall_summary.parquet", "System call summaries"),
        
        // Moonshine
        ("data/moonshine/elf_moonshine_map.txt", "Modular forms in ELF binaries"),
        
        // Registry
        ("data/registry.json", "Central data registry"),
    ];
    ...
    // Create README
    let readme = format!(r#"# meta-introspector: Unified Code Analysis Dataset
...
"#, generated = chrono::Utc::now().to_rfc3339());
    ...
    Command::new("huggingface-cli")
        .args(["upload", repo, "README.md", "--repo-type", "dataset"])
        .output()?;
    
    println!("\n✅ COMPLETE!");
    println!("   Pushed: {}", pushed);
    println!("   Skipped: {}", skipped);
    println!("\n🌐 View at: https://huggingface.co/datasets/{}", repo);
