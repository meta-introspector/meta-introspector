...
fn main() {
    ...
    // Read sorted repos from file
    let repos_file = "data/raw/git_repos_by_date.txt";
    let content = fs::read_to_string(repos_file)
        .expect("Failed to read git_repos_by_date.txt");
    
    // Parse repos from last month
    let one_month_ago = Utc::now() - Duration::days(30);
    let cutoff_timestamp = one_month_ago.timestamp();
    
    let recent_repos: Vec<String> = content
        .lines()
        .filter_map(|line| {
    ...
    println!("Found {} repositories with activity in last month", recent_repos.len());
    ...
    println!("Processed {} recent repositories with {} total commits to {}", 
