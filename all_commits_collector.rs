...
impl CommitQueue {
    ...
    pub fn process_next(&mut self, target_author: &str) -> Option<()> {
        if let Some(mut job) = self.jobs.pop_front() {
            ...
            match collect_all_commits(&job.repo_path, &job.repo_name, target_author) {
                Ok(repo_commits) => {
                    if repo_commits.total_commits > 0 {
    ...
    pub fn save_results(&self) -> Result<(), Box<dyn std::error::Error>> {
        // Save individual results for repos with commits
        for repo_commits in &self.completed {
            if repo_commits.total_commits > 0 {
                let result_file = format!(
                    "/mnt/data1/meta-introspector/data/processed/all_commit_results/{}.json",
        ...
        // Save summary
        let repos_with_commits = self.completed.iter().filter(|r| r.total_commits > 0).count();
        let total_commits: usize = self.completed.iter().map(|r| r.total_commits).sum();
        let successful_fetches = self.completed.iter().filter(|r| r.remotes_fetched).count();
        ...
        let summary = serde_json::json!({
            ...
            "successful_remote_fetches": successful_fetches,
            "failed_repos": self.failed.len()
        });
        
        fs::write(
            "/mnt/data1/meta-introspector/data/processed/all_commits_summary.json",
...
fn collect_all_commits(
    ...
    match fetch_output {
        Ok(output) if output.status.success() => {
            remotes_fetched = true;
        }
        Ok(output) => {
            let error = String::from_utf8_lossy(&output.stderr);
            fetch_errors.push(format!("Fetch failed: {}", error));
        }
        Err(e) => {
            fetch_errors.push(format!("Fetch error: {}", e));
        }
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🚀 ALL COMMITS COLLECTION SYSTEM");
    println!("================================");
    ...
    println!("📡 Fetching all remotes and collecting commits...");
    ...
    let repos_with_commits = queue.completed.iter().filter(|r| r.total_commits > 0).count();
    let total_commits: usize = queue.completed.iter().map(|r| r.total_commits).sum();
    let successful_fetches = queue.completed.iter().filter(|r| r.remotes_fetched).count();
    
    println!("\n🎉 COLLECTION COMPLETE!");
    println!("📊 Results:");
    println!("   - Total repos processed: {}", queue.completed.len());
    println!("   - Repos with your commits: {}", repos_with_commits);
    println!("   - Your total commits (past month): {}", total_commits);
    println!("   - Successful remote fetches: {}", successful_fetches);
    println!("   - Failed repos: {}", queue.failed.len());
    
    queue.save_results()?;
    println!("💾 Results saved to data/processed/all_commit_results/");
