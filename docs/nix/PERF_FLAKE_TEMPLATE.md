...
```nix
{
  ...
  inputs = {
    ...
    perf-lib.url = "path:../../nix/perf-lib.nix";  # Import standard library
  };
...
```nix
{
  inputs.perf-lib.url = "path:../../nix/perf-lib.nix";
...
```bash
#!/bin/bash
# Use standard perf library
nix run ../../nix/perf-lib.nix#perf-build -- .#target
```
