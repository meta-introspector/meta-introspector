    ...
    () => {{
        ...
        if let Ok(mut file) = OpenOptions::new().create(true).append(true).open(&log_file) {
            writeln!(file, r#"{{\"type\":\"init\",\"message\":\"All call wrappers initialized\",\"timestamp\":{},\"project\":\"{}\",\"binaries\":{},\"libraries\":{},\"symbols\":{}}}\"#, 
                    timestamp, project, 14, 39, 456).ok();
        }
        
        println!("  1. ld ({} libs, {} syms)", 0, 0);
        println!("  2. as ({} libs, {} syms)", 2, 50);
        println!("  3. rustc ({} libs, {} syms)", 0, 3);
        println!("  4. gcc ({} libs, {} syms)", 2, 50);
        println!("  5. sh ({} libs, {} syms)", 2, 50);
        println!("  6. cc ({} libs, {} syms)", 0, 0);
        println!("  7. gcc ({} libs, {} syms)", 3, 50);
        println!("  8. cc ({} libs, {} syms)", 0, 0);
        println!("  9. ld ({} libs, {} syms)", 6, 50);
        println!("  10. readlink ({} libs, {} syms)", 5, 50);
        println!("  11. ld ({} libs, {} syms)", 2, 50);
        println!("  12. cc ({} libs, {} syms)", 0, 50);
        println!("  13. collect2 ({} libs, {} syms)", 3, 50);
        println!("  14. rustc ({} libs, {} syms)", 14, 3);
